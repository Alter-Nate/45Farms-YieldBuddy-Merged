...
#			time.sleep(2)
	...
	#Read Serial Device and Update according to the information that was received.
		...
		#If the 'timesync' counter value goes over 20, then update the Raspberry Pi's time to be that of the Arduino's.
			...
			print("\033[20;0H\r")
			print("\033[20;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Sensors: %s,%s,%s,%s,%s,%s,%s,%s,%s"%(pH1,pH2,Temp,RH,TDS1,TDS2,CO2,Light,Water))
			now = datetime.now()
			...
			if (delta >= TakeDataPoint_Every) or (Datapoint_count == 0 and first_timesync == True):
				...
				printMessageLog()
				update_sql("INSERT INTO 'Sensors_Log' (Time,pH1,pH2,Temp,RH,TDS1,TDS2,CO2,Light,Water) VALUES ('" + now.strftime("%Y-%m-%d %H:%M:%S") + "'," + pH1 + "," + pH2+ "," + Temp + "," + RH + "," + TDS1 + "," + TDS2 + "," + CO2 + "," + Light  + "," + Water + ")")
				LastDataPoint_Time = datetime.now()
				timesync = 0 #do a timesync
				Datapoint_count = Datapoint_count + 1
			#SENSOR VALUES
			update_sql("UPDATE `Sensors` SET pH1 = " + pH1 + ", pH2 = " + pH2+ ", Temp = " + Temp + ", RH = " + RH + ", TDS1 =" + TDS1 + ", TDS2 =" + TDS2 + ", CO2 = " + CO2 + ", Light = " + Light + ", Water = " + Water)
			db.commit()
		elif 'Relays' in line:
			if oldRelays != line:
				oldRelays = line
