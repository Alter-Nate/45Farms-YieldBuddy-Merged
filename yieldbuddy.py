    ...
    try:
        ...
        if Command != '':
            ...
            if 'restart cam' in Command:
                ...
                ser.close()
                f = os.system(
                    "sudo avrdude -V -F -c avrisp2 -p m2560 -P /dev/ttyACM1 -U flash:w:" + app_path + "/upload/firmware.cpp.hex")
        ...
        print(
            "\033[35;0H                                                                                                           ")
        print("\033[35;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Now: " + now.strftime(
            '%s') + "  Last Data Point: " + LastDataPoint_Time.strftime('%s') + "   Next Data Point [sec]: " + str(
            float(now.strftime('%s')) - float(LastDataPoint_Time.strftime('%s'))) + "/" + str(
            TakeDataPoint_Every - (float(now.strftime('%s')) - float(LastDataPoint_Time.strftime('%s')) )) + "/" + str(
            TakeDataPoint_Every))
        ...
        if 'Time' in line:
            ...
            if ArduinoTime != '':
                ...
                print '\033[1m'  #Bold
                print(
                    "\033[19;0H[Arduino Time: " + ArduinoTime + "]------------------------[Raspberry Pi Time: " + now.strftime(
                        "%b %d %Y %I:%M:%S %p") + "]")
                print '\033[0m'  #Un-Bold
                update_sql(
                    "UPDATE `Arduino` SET `Time` = '" + ArduinoTime + "' , Month=" + Arduino_month + ", Day=" + Arduino_day + ", Year=" + Arduino_year + ", Hour=" + Arduino_hour + ", Minute=" + Arduino_min + ", Second=" + Arduino_sec)
            #If the 'timesync' counter value goes over 20, then update the Raspberry Pi's time to be that of the Arduino's.
            if timesync > 20:
                try:
                    f = os.system(
                        "sudo date " + Arduino_month + Arduino_day + Arduino_hour + Arduino_min + Arduino_year + "." + Arduino_sec + " >/dev/null 2>&1")
            ...
            Sensors, pH1, pH2, Temp, RH, TDS1, TDS2, CO2, Light, Water = line.split(",")
            Sensors = Sensors.replace("Read fail", "")
            Light = Light.rstrip()
            elapsedTime = now - startTime
            elapsedSeconds = (elapsedTime.microseconds + (
                elapsedTime.days * 24 * 3600 + elapsedTime.seconds) * 10 ** 6) / 10 ** 6
            print("\033[20;0H\r")
            print("\033[20;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Sensors: %s,%s,%s,%s,%s,%s,%s,%s,%s" % (
                pH1, pH2, Temp, RH, TDS1, TDS2, CO2, Light, Water))
            now = datetime.now()
            ...
            if (delta >= TakeDataPoint_Every) or (Datapoint_count == 0 and first_timesync == True):
                ...
                printMessageLog()
                update_sql(
                    "INSERT INTO 'Sensors_Log' (Time,pH1,pH2,Temp,RH,TDS1,TDS2,CO2,Light,Water) VALUES ('" + now.strftime(
                        "%Y-%m-%d %H:%M:%S") + "'," + pH1 + "," + pH2 + "," + Temp + "," + RH + "," + TDS1 + "," + TDS2 + "," + CO2 + "," + Light + "," + Water + ")")
                LastDataPoint_Time = datetime.now()
                timesync = 0  #do a timesync
                Datapoint_count = Datapoint_count + 1
            #SENSOR VALUES
            update_sql(
                "UPDATE `Sensors` SET pH1 = " + pH1 + ", pH2 = " + pH2 + ", Temp = " + Temp + ", RH = " + RH + ", TDS1 =" + TDS1 + ", TDS2 =" + TDS2 + ", CO2 = " + CO2 + ", Light = " + Light + ", Water = " + Water)
            db.commit()
        elif 'Relays' in line:
            if oldRelays != line:
                oldRelays = line
                ...
                print(
                    "\033[21;0H                                                                                                                       ")
                print("\033[21;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Relays: %s,%s,%s,%s,%s,%s,%s,%s" % (
                    Relay1, Relay2, Relay3, Relay4, Relay5, Relay6, Relay7, Relay8))
                #RELAYS
                update_sql(
                    "UPDATE `Relays` SET Relay1 = '" + Relay1 + "', Relay2 = '" + Relay2 + "', Relay3 = '" + Relay3 + "', Relay4 = '" + Relay4 + "', Relay5 = '" + Relay5 + "', Relay6 = '" + Relay6 + "'" + Relay7 + "'" + Relay8 + "'")
                db.commit()
            ...
            if oldRelay_isAuto != line:
                ...
                print(
                    "\033[22;0H                                                                                                                       ")
                print("\033[22;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Relay_isAuto: %s,%s,%s,%s,%s,%s" % (
                    Relay1_isAuto, Relay2_isAuto, Relay3_isAuto, Relay4_isAuto, Relay5_isAuto, Relay6_isAuto, Relay7_isAuto, Relay8_isAuto))
                #RELAYS
                update_sql(
                    "UPDATE `Relays` SET Relay1_isAuto = " + Relay1_isAuto + ", Relay2_isAuto = " + Relay2_isAuto + ", Relay3_isAuto = " + Relay3_isAuto + ", Relay4_isAuto = " + Relay4_isAuto + ", Relay5_isAuto =" + Relay5_isAuto + ", Relay6_isAuto =" + Relay6_isAuto + ", Relay7_isAuto =" + Relay7_isAuto + ", Relay8_isAuto =" + Relay8_isAuto)
                db.commit()
            ...
            if oldLight_Schedule != line:
                ...
                print(
                    "\033[23;0H                                                                                                                       ")
                print("\033[23;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Light_Schedule: %s,%s,%s,%s" % (
                    Light_ON_hour, Light_ON_min, Light_OFF_hour, Light_OFF_min))
                #LIGHTING
                update_sql(
                    "UPDATE `Light_Schedule` SET Light_ON_hour = " + Light_ON_hour + ", Light_ON_min = " + Light_ON_min + ", Light_OFF_hour = " + Light_OFF_hour + ", Light_OFF_min = " + Light_OFF_min)
            ...
            if oldWatering_Schedule != line:
                ...
                print(
                    "\033[24;0H                                                                                                                       ")
                print(
                    "\033[24;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Watering_Schedule: %s,%s,%s,%s,%s,%s,%s" % (
                        Pump_start_hour, Pump_start_min, Pump_start_isAM, Pump_every_hours, Pump_every_mins, Pump_for,
                        Pump_times))
                #WATERING
                update_sql(
                    "UPDATE `Watering_Schedule` SET Pump_start_hour = " + Pump_start_hour + ", Pump_start_min = " + Pump_start_min + ", Pump_start_isAM = " + Pump_start_isAM + ", Pump_every_hours = " + Pump_every_hours + ", Pump_every_mins =" + Pump_every_mins + ", Pump_for =" + Pump_for + ", Pump_times =" + Pump_times)
            ...
            if oldSetPoint_pH1 != line:
                ...
                print(
                    "\033[25;0H                                                                                                                       ")
                print("\033[25;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_pH1: %s,%s,%s" % (
                    pH1Value_Low, pH1Value_High, pH1_Status))
                #SetPoint_pH
                update_sql(
                    "UPDATE `pH1` SET Low='" + pH1Value_Low + "', High='" + pH1Value_High + "', Status='" + pH1_Status + "'")
                pH1_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM pH1")
                pH1_High_Alarm = fetch_sql("SELECT High_Alarm FROM pH1")
                if pH1_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "pH1 Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql(
                            "UPDATE `pH1` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if pH1_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "pH1 High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `pH1` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in pH1_Status and pH1_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `pH1` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in pH1_Status and pH1_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `pH1` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_pH2 != line:
                ...
                print(
                    "\033[26;0H                                                                                                                       ")
                print("\033[26;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_pH2: %s,%s,%s" % (
                    pH2Value_Low, pH2Value_High, pH2_Status))
                #SetPoint_pH
                update_sql(
                    "UPDATE `pH2` SET Low='" + pH2Value_Low + "',High='" + pH2Value_High + "',Status='" + pH2_Status + "'")
                pH2_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM pH2")
                pH2_High_Alarm = fetch_sql("SELECT High_Alarm FROM pH2")
                if pH2_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "pH2 Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql(
                            "UPDATE `pH2` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if pH2_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "pH2 High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `pH2` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in pH2_Status and pH2_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `pH2` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in pH2_Status and pH2_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `pH2` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_Temp != line:
                ...
                print(
                    "\033[27;0H                                                                                                                       ")
                print("\033[27;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Temp: %s,%s,%s,%s,%s,%s,%s" % (
                    TempValue_Low, TempValue_High, Heater_ON, Heater_OFF, AC_ON, AC_OFF, Temp_Status))
                #SetPoint_pH
                update_sql(
                    "UPDATE `Temp` SET Low = " + TempValue_Low + ", High = " + TempValue_High + ", Heater_ON = " + Heater_ON + ", Heater_OFF = " + Heater_OFF + ", AC_ON =" + AC_ON + ", AC_OFF =" + AC_OFF + ", Status ='" + Temp_Status + "'")
                ...
                if Temp_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "Temp Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `Temp` SET Low_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if Temp_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "Temp High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `Temp` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in Temp_Status and Temp_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `Temp` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in Temp_Status and Temp_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `Temp` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_RH != line:
                ...
                print(
                    "\033[28;0H                                                                                                                       ")
                print("\033[28;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_RH: %s,%s,%s,%s,%s,%s,%s" % (
                    RHValue_Low, RHValue_High, Humidifier_ON, Humidifier_OFF, Dehumidifier_ON, Dehumidifier_OFF,
                    RH_Status))
                #SetPoint_RH
                update_sql(
                    "UPDATE `RH` SET Low = " + RHValue_Low + ", High = " + RHValue_High + ", Humidifier_ON = " + Humidifier_ON + ", Humidifier_OFF = " + Humidifier_OFF + ", Dehumidifier_ON =" + Dehumidifier_ON + ", Dehumidifier_OFF =" + Dehumidifier_OFF + ", Status ='" + RH_Status + "'")
                RH_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM RH")
                RH_High_Alarm = fetch_sql("SELECT High_Alarm FROM RH")
                if RH_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "RH Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql(
                            "UPDATE `RH` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if RH_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "RH High", smtp_server,
                                         smtp_port) == 1:
                        update_sql(
                            "UPDATE `RH` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in RH_Status and RH_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `RH` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in RH_Status and RH_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `RH` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_TDS1 != line:
                ...
                print(
                    "\033[29;0H                                                                                                                       ")
                print("\033[29;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_TDS1: %s,%s,%s,%s,%s,%s" % (
                    TDS1Value_Low, TDS1Value_High, NutePump1_ON, NutePump1_OFF, MixPump1_Enabled, TDS1_Status))
                #SetPoint_TDS1
                update_sql(
                    "UPDATE `TDS1` SET Low = " + TDS1Value_Low + ", High = " + TDS1Value_High + ", NutePump1_ON = " + NutePump1_ON + ", NutePump1_OFF = " + NutePump1_OFF + ", MixPump1_Enabled =" + MixPump1_Enabled + ", Status ='" + TDS1_Status + "'")
                ...
                if TDS1_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "TDS1 Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `TDS1` SET Low_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if TDS1_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "TDS1 High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `TDS1` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in TDS1_Status and TDS1_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `TDS1` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in TDS1_Status and TDS1_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `TDS1` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_TDS2 != line:
                ...
                print(
                    "\033[30;0H                                                                                                                       ")
                print("\033[30;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_TDS2: %s,%s,%s,%s,%s,%s" % (
                    TDS2Value_Low, TDS2Value_High, NutePump2_ON, NutePump2_OFF, MixPump2_Enabled, TDS2_Status))
                #SetPoint_TDS2
                update_sql(
                    "UPDATE `TDS2` SET Low = " + TDS2Value_Low + ", High = " + TDS2Value_High + ", NutePump2_ON = " + NutePump2_ON + ", NutePump2_OFF = " + NutePump2_OFF + ", MixPump2_Enabled =" + MixPump2_Enabled + ", Status ='" + TDS2_Status + "'")
                ...
                if TDS2_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "TDS2 Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `TDS2` SET Low_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if TDS2_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "TDS2 High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `TDS2` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in TDS2_Status and TDS2_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `TDS2` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in TDS2_Status and TDS2_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `TDS2` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_CO2 != line:
                ...
                print(
                    "\033[31;0H                                                                                                                       ")
                print("\033[31;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_CO2: %s,%s,%s,%s,%s,%s" % (
                    CO2Value_Low, CO2Value_High, CO2_ON, CO2_OFF, CO2_Enabled, CO2_Status))
                #SetPoint_CO2'
                update_sql(
                    "UPDATE `CO2` SET Low = " + CO2Value_Low + ", High = " + CO2Value_High + ", CO2_ON = " + CO2_ON + ", CO2_OFF = " + CO2_OFF + ", CO2_Enabled =" + CO2_Enabled + ", Status = '" + CO2_Status + "'")
                CO2_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM CO2")
                CO2_High_Alarm = fetch_sql("SELECT High_Alarm FROM CO2")
                if CO2_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "CO2 Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql(
                            "UPDATE `CO2` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if CO2_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "CO2 High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `CO2` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in CO2_Status and CO2_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `CO2` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in CO2_Status and CO2_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `CO2` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ...
            if oldSetPoint_Light != line:
                ...
                print(
                    "\033[32;0H                                                                                                           ")
                print("\033[32;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Light: %s,%s,%s" % (
                    LightValue_Low, LightValue_High, Light_Status))
                #SetPoint_pH
                update_sql(
                    "UPDATE `Light` SET Low = '" + LightValue_Low + "', High = '" + LightValue_High + "', Status = '" + Light_Status + "'")
                ...
                if Light_Low_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "Light Low", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `Light` SET Low_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if Light_High_Alarm[0] == 1:
                    if email_message_tls(login_address, email_password, to_address, "Light High", smtp_server,
                                         smtp_port) == 1:
                        update_sql("UPDATE `Light` SET High_Alarm = 2, Low_Time = '" + now.strftime(
                            "%b %d %Y %I:%M:%S %p") + "'")
                if 'LOW' in Light_Status and Light_Low_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `Light` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
                if 'HIGH' in Light_Status and Light_High_Alarm[0] == 0:
                    update_sql(
                        "UPDATE `Light` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
            ser.flushInput()
