...
//Water
$Light_query = "SELECT Water FROM Sensors ORDER BY `Time` DESC ";
$Light_result = $mysqli->query($Water_query);     
if (!$Water_result) {
  printf("Query failed: %s\n", $mysqli->error);
  exit;
}

$Water_row = $Water_result->fetch_row();
$Water_row = implode(" ", $Water_row);
echo $Water_row;
echo "&nbsp;&nbsp;&nbsp;";
$_SESSION['Sensors_Light'] = $Water_row;
$Water_result->close();
